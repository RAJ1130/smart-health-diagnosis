...
# Comprehensive medical database with multiple treatment variations
MEDICAL_CONDITIONS = {
    'common_cold': {
    ...
    'influenza': {
        ...
        'treatment_variations': [
            ...
            [
                "SYMPTOM RELIEF: TheraFlu according to package directions",
                "BODY ACHES: Epsom salt baths + Magnesium 400mg daily",
                "HEADACHE: Acetaminophen 500mg + Caffeine 65mg combination",
                "CHEST CONGESTION: Guaifenesin 600mg every 12 hours + Chest percussion",
                "FATIGUE: Vitamin C 1000mg daily + B-complex vitamin daily",
                "ISOLATION: Stay home until fever-free 24h without medication"
            ]
        ],
    ...
    'covid_19': {
        ...
        'treatment_variations': [
            ...
            [
                "MONITORING: Pulse oximeter checks + Daily symptom diary",
                "FEVER: Ibuprofen 400mg every 6 hours if no contraindications",
                "COUGH: Guaifenesin 600mg every 12 hours for chest congestion",
                "GASTROINTESTINAL: Loperamide 4mg initially then 2mg after loose stool",
                "HYDRATION: Electrolyte solutions 2-3L daily + Clear broths",
                "FOLLOW-UP: Telehealth visit in 5-7 days for reassessment"
            ]
        ],
    ...
    'migraine': {
        ...
        'treatment_variations': [
            ...
            [
                ...
                "NAUSEA: Ondansetron 4mg ODT every 8 hours if vomiting",
                "NON-PHARMACOLOGIC: Biofeedback + Acupuncture + Magnesium 400mg daily",
                "MENSTRUAL: Naproxen 550mg twice daily starting 2 days before period",
                "PREVENTIVE: Amitriptyline 10-50mg at bedtime",
                "LIFESTYLE: Regular meals + Hydration + Consistent sleep schedule"
            ]
        ],
        'severity': 'Moderate to Severe',
        'icon': '😵',
        ...
        <section class="hero">
            <div class="container">
                <div class="hero-content">
                    <h1>MediScan Pro</h1>
                    <p>Advanced AI-powered health diagnosis with varied treatment plans across 200+ symptoms and 50+ medical conditions</p>
        ...
        <div class="container">
            <div class="assessment-main">
                ...
                <div class="symptoms-panel">
                    ...
                    <p style="color: var(--gray); margin-bottom: 25px;">Select all symptoms you're experiencing. Emergency symptoms are highlighted in red. Our AI analyzes 200+ symptoms across 50+ conditions with varied treatment plans.</p>
    ...
    html += f'''
        ...
        <script>
                ...
                Array.from(selectedSymptoms).forEach(symptom => {{
                    ...
                    html += `
                        <div class="selected-item ${{emergencyClass}}">
                            ...
                            <button type="button" class="remove-btn" onclick="removeSymptom('${{symptom.replace(/'/g, "\\\\'")}}')">
                ...
                try {{
                    const response = await fetch('/diagnose', {{
                        ...
                        body: JSON.stringify({{ 
                            ...
                            previous_plan: lastTreatmentPlan
                        }})
                    }});
                    ...
                    lastTreatmentPlan = result.treatment_plan_index;
                    ...
                    <div class="result-card">
                        ...
                        <div style="margin-bottom: 25px;">
                            <h3 style="margin-bottom: 15px; color: var(--dark);">
                                <i class="fas fa-prescription-bottle-medical"></i> Treatment Plan Variation #${{result.treatment_plan_index + 1 || 1}}
                            </h3>
                        ...
                        <div style="margin-top: 20px; text-align: center;">
                            <button class="btn btn-secondary" onclick="requestAlternativePlan()" style="margin: 10px;">
                                <i class="fas fa-sync-alt"></i> Get Alternative Treatment Plan
                            </button>
                            <button class="btn btn-primary" onclick="saveTreatmentPlan()" style="margin: 10px;">
                        ...
                        <div style="margin-top: 30px; padding: 20px; background: #f8fafc; border-radius: 12px; border-left: 4px solid var(--warning);">
                            <p style="color: var(--gray); font-size: 0.9rem;">
                                <strong>Medical Disclaimer:</strong> This assessment provides alternative treatment approaches. Different plans may work better for different individuals. 
            ...
            function requestAlternativePlan() {{
                // Trigger a new assessment to get a different treatment variation
                performAssessment();
            }}

            function saveTreatmentPlan() {{
                alert('Treatment plan saved to your records. Always consult with your healthcare provider before starting any new treatment.');
            }}
        </script>
    </body>
    </html>
    ...
    try:
        ...
        if emergency_symptoms:
            emergency_treatments = [
                ...
                [
                    "CARDIAC: Nitroglycerin 0.4mg under tongue if prescribed (max 3 doses)",
                    "ANAPHYLAXIS: Epinephrine IM in thigh + Repeat in 5-15 minutes if needed",
                    "TRAUMA: Immobilize injured area + Control bleeding with pressure",
                    "OVERDOSE: Naloxone if opioid overdose suspected + Rescue breathing",
                    "Keep emergency contacts and medical information easily accessible"
                ]
            ]
            
            treatment_index = random.randint(0, len(emergency_treatments) - 1)
            if previous_plan is not None:
                treatment_index = (previous_plan + 1) % len(emergency_treatments)
        ...
        if best_match and best_score >= 40:
            ...
            # Select treatment variation
            treatment_variations = condition.get('treatment_variations', [condition.get('treatment', [])])
            if not isinstance(treatment_variations[0], list):
                treatment_variations = [treatment_variations]
            
            treatment_index = random.randint(0, len(treatment_variations) - 1)
            if previous_plan is not None:
                treatment_index = (previous_plan + 1) % len(treatment_variations)
            ...
            # Return general advice for unclear matches with variations
            general_treatments = [
                [
                    "FEVER: Acetaminophen 650mg every 6 hours (max 3000mg/day) + Cool compresses",
                    "PAIN: Ibuprofen 400mg every 6 hours with food (max 1200mg/day)",
                    "NAUSEA: Ginger tea + Small frequent meals + Avoid strong odors",
                    "FATIGUE: Rest + Hydration + Gradual return to activity",
                    "COUGH: Honey in warm tea + Humidifier + Elevate head during sleep",
                    "MONITOR: Temperature every 4 hours + Watch for worsening symptoms"
                ],
                [
                    "SYMPTOM RELIEF: Multi-symptom cold/flu medication as directed",
                    "HYDRATION: Electrolyte solutions + Clear broths + Herbal tea",
                    "COMFORT: Warm baths + Steam inhalation + Throat lozenges",
                    "NUTRITION: BRAT diet (bananas, rice, applesauce, toast) if GI symptoms",
                    "IMMUNE SUPPORT: Vitamin C 1000mg + Zinc 15mg daily",
                    "SEEK CARE: If no improvement in 48 hours or new symptoms develop"
                ],
                [
                    "PAIN MANAGEMENT: Acetaminophen-Ibuprofen alternation every 3 hours",
                    "RESPIRATORY: Saline nasal spray + Guaifenesin for congestion",
                    "GASTROINTESTINAL: Probiotics + BRAT diet + Clear fluids",
                    "SKIN: Calamine lotion for rashes + Cool compresses for swelling",
                    "REST: 7-9 hours sleep + Light activity as tolerated",
                    "FOLLOW-UP: Primary care visit within 3-5 days if persistent"
                ]
            ]
            
            treatment_index = random.randint(0, len(general_treatments) - 1)
            if previous_plan is not None:
                treatment_index = (previous_plan + 1) % len(general_treatments)
            ...
            return jsonify({
                ...
                'contagious_period': 'Practice good hygiene until diagnosis confirmed',
                'treatment_plan_index': treatment_index
            })
